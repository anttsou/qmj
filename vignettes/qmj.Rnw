...
\section{quality}
  Finally, we have the main dataset, \code{quality}, which contains the quality scores for each company as well as the four z-scores measuring profitability, growth, safety, and payouts that make up the quality score that were calculated using the methodology descried above.
