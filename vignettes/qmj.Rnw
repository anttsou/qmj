...
The second dataset, \code{financials} contains the data from the balance sheets, income statements, and cash flows of companies that is necessary for quality score generation.
...
\subsection{Prices}
The \code{prices} dataset has the closing stock prices and price returns for each day of the past two years for each company. Here is a sample of the data:
...
Above, we see data for 1st Source Corporation(with ticker SRCE) and the Standard and Poor's 500 Index(with ticker GSPC). 

\subsection{Quality}
The data frame \code{quality} contains the quality scores for each company as well as the four z-scores measuring profitability, growth, safety, and payouts that make up the quality score following the methodology described below on the data in the \code{prices} and the \code{financials} data sets.
...
\strong{qmj} is planned to be updated periodically with up-to-date data, especially as the Russell 3000 index rebalances. However, the users can also call the functions that generate the dataframes to. This may be useful for users who perhaps want to run their own list of US tickers, or want frequent updates based on recent returns.

\strong{get\_companies()} will create a companies data set from a text file. Usually this information will only need to be updated once a year, as the index rebalances. The user will have to provide the new text file however. \strong{get\_prices()} takes a data frame of companies from \strong{get\_companies()}, organized by name and ticker, and returns the daily prices and returns for the past two years including the most recent trading day. \strong{get\_info()} also takes a data frame of companies, organized by name and ticker, and grabs the most recent company 10-K financial statements. \strong{get\_info()} does not need to be called often since it will only grab new data once per year. 
...
An easy way to make the data more readable is through tidy functions in the \strong{qmj} package. 
...
\strong{tidy\_prices()} takes as input the result of \strong{get\_prices()} and \strong{tidyinfo()} takes as input the result of \strong{get\_info()}. The column names of these data frames will be the same abbreviations that are used in the default data frames described in Input Data section. Then the quality data frame can be generated using \strong{market\_data()}.
...
The \strong{qmj} package offers an R implementation of the methodologies found in the paper,``Quality Minus Junk''\citep{asness:2013}. The quality scores data it comes with, as well as the functions it provides to calculate the data can be used by a variety of users with different needs. 

A limitation is that it itself can not provide time-series data of these quality scores. An extension as such to the package could provide useful in testing long-quality, short-junk strategies in backtests\footnote{See \cite{enos:2013} for more information}. Such an extension however, would be difficult while still depending on openly available data on Google Finance and Yahoo Finance.

Regardless, \strong{qmj} provides useful tools to create a data frame that provides quality scores, as well as the profitability, growth and safety payout subscores on a ticker level for the Russell 3000 Universe. As it comes with all the dataframes pre-generated, it is also a useful reference tool. 
