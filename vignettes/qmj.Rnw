...
\citep{lu:2013}
\abstract{
The \strong{qmj} package produces quality scores for companies from the Russell 3000 Index based on their profitability, growth, safety, and payouts, using the approach described in "Quality Minus Junk" by Clifford Asness et. al (2013). The package gathers the latest company financial statements and stock returns from Google Finance and provides periodically updated quality scores. Users also have access to the functions used to generate these scores, allowing them to update their data whenever desired. The package helps separate the list of companies into ``junk'' stocks, which are expected to underperform relative to the market, and ``quality'' stocks, which are expected to outperform.
}

\section{Introduction}

A Quality Minus Junk approach \citep{asness:2013}, involves investing long on high-quality stocks and shorting low-quality stocks to produce high risk-adjusted returns. A high-quality stock has characteristics investors should be willing to pay for, ceteris paribus. These characteristics are profitability, growth, safety, and payouts. Thus, the return of a quality stock is intended to exceed its purchase price. 

Each of the four components that make up the quality score are calculated from different variables that can be found from a company's financial statements.  The package draws this data from Google Finance.

The \strong{qmj} package evaluates the quality of stocks by measuring the profitability, growth, safety, and payouts of a company relative to other companies in the Russell 3000 Index. We define each of these characteristics in the following section of the paper. 
