...
\subsection{prices}
The \code{prices} dataset has the closing stock prices and price returns for each day of the past two years for each company. Similar to \code{get\_info}, \code{get\_prices} gets daily price information for stocks using the \strong{quantmod} package, but this time using Yahoo Finance. Here is a sample of the data:
...
Above, we see data for 1st Source Corporation(with ticker SRCE) and the Standard and Poor's 500 Index(with ticker GSPC). For every company in the dataset, we have the daily price return (pret) and closing price (close), and we compare these values to those of the market to see if the company outperforms or underperformsthe market. The S \& P 500 is the benchmark we use in our calculations of beta and idiosyncratic volatility for our safety score. Here, SRCE underperforms the market for every day in our sample of five consecutive business days.
...
The \strong{qmj} package offers an R implementation of the assigning quality scores found in the paper,``Quality Minus Junk''\citep{asness:2013} for the Russell 3000. The quality data frame it comes with, as well as the functions it provides to calculate the data can be used by a variety of users with different needs. 

A limitation is that it does not provide time-series data of these quality scores. An extension as such to the package could make it more useful. It would enable the backtesting of quality scores as a predictive factor for future returns\footnote{See \cite{enos:2013} for more information}.
