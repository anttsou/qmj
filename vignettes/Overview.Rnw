...
In order to start you off, qmj comes equipped with several data sets, including company information, financial statements, and daily stock data. These data sets are named:

\begin{lstlisting}[language=R]
companies
tidaydaily  #stores daily stock data for the past 5 years, if possible.
tidybalance
tidycash
tidyincome
\end{lstlisting}
...
\end{lstlisting}
This will return a numeric vector containing profitability z-scores for the given companies, where the nth number corresponds to the nth company. If you're interested in inputting your own data, you must follow the organization of the tidy data sets, though these can be generated from a user-created company data file as follows:
\begin{lstlisting}[language=R]
...
Although pre-loaded data already exists as part of the package, it's easy to update your local data sets. Simply call
\begin{lstlisting}[language=R]
getinfo()
\end{lstlisting}
To save the most recent 4 years of financial statements for the companies.RData file in the extdata folder.
